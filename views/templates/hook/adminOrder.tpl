...
{else}
	{assign var="total_shipping_tax_incl" value=Tools::convertPrice($order->total_shipping_tax_incl, $order->id_currency, false)}
{/if}
<br />
...
<fieldset id="dpdgeopost">
	...
	{if $settings->checkRequiredFields()}
		<div id="dpdgeopost_notice_container"{if isset($ps14) && $ps14} class="order_ps14"{/if}>
			{if isset($errors) && $errors}
				{include file=$smarty.const._PS_MODULE_DIR_|cat:'dpdgeopost/views/templates/admin/errors.tpl'}
			{elseif !$shipment->id_shipment && !$selected_shipping_method_id}
				<p class="warn">{l s='Client did not selected DPD shipment, but you can use this shipment method.' mod='dpdgeopost'}</p>
			{elseif $ws_shippingPrice > 0 && $ws_shippingPrice > $total_shipping_tax_incl}
		...
		<table width="100%" cellspacing="0" cellpadding="0" class="table{if isset($ps14) && $ps14} order_ps14{/if}">
			...
			<tbody>
				<tr>
					...
					<td>{if $shipment->parcels}{$shipment->parcels|count|escape:'htmlall':'UTF-8'}{elseif $settings->packaging_method == DpdGeopostConfiguration::ONE_PRODUCT}{$products|count}{else}1{/if}</td>
					<td><span id="dpdgeopost_paid_price"{if $ws_shippingPrice > 0 && $ws_shippingPrice > $total_shipping_tax_incl} style="color:red"{/if}>{displayPrice price=$total_shipping_tax_incl}</span></td>
		...
		{else}
			<input type="button" class="button"{if !$selected_shipping_method_id || $ws_shippingPrice == '---'} disabled="disabled"{/if} id="dpdgeopost_create_shipment" value="{l s='Create shipment' mod='dpdgeopost'}" autocomplete="off" />
		{/if}
		<div class="clear"></div>
					...
					<tr>
						...
						<td>
							<select class="parcel_selection" autocomplete="off">
								{if isset($product.parcelReferenceNumber)}
									{foreach from=$shipment->parcels key=parcel_no item=parcel}
										{if $parcel.parcelReferenceNumber == $product.parcelReferenceNumber}
	...
	{else}
		<p class="warn">{l s='Please provide required information in module settings page' mod='dpdgeopost'} <a href="{$module_link|escape:'htmlall':'UTF-8'}&menu=configuration">{l s='here' mod='dpdgeopost'}</a></p>
	{/if}
