...
class DpdGeopostCSVController extends DpdGeopostController
{
	...
	private function validateCSVData($csv_data)
	{
		...
		if ($method_validation !== true)
			$errors[] = sprintf(
				$this->l('Method ID can be declared one value of: %1$d, %2$d, %3$d, %4$d - invalid lines: %5$s'),
				_DPDGEOPOST_CLASSIC_ID_, _DPDGEOPOST_10_ID_, _DPDGEOPOST_12_ID_, _DPDGEOPOST_SAME_DAY_ID_, $method_validation
			);
	...
	{
		$available_methods = array(_DPDGEOPOST_CLASSIC_ID_, _DPDGEOPOST_10_ID_, _DPDGEOPOST_12_ID_, _DPDGEOPOST_SAME_DAY_ID_, '*');
		$wrong_methods = '';
		for ($i = 0; $i < $csv_data_count; $i++)
